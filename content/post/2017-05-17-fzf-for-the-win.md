...
My `zsh` configuration tells zsh to write all the commands I type in a `~/.zshhistory` file
(with timestamps)
...
For instance, if I've typed the following list
...
There's an other trick you can use with zsh. Instead of listing all the commands
in chronological order, you can tell it to only display commands who start as
your current line.
...
and then select the correct `ssh` command.
...
Rather that hitting `CTRL-P` a bunch of times, I would use `CTRL-R` instead.
...
workflow. Look for patterns and then find one way to deal with it that you can
re-use everywhere.
...
`fzf` does only one thin and it does it well. You are supposed to use it with a
...
Here `fzf` will use a few lines of text allowing you to select one of the
elements returned by the  `some-source` command.

While you are typing the pattern to filter the list of matches, it will update
...
* It's cross-platform
* It can handle asynchrone stuff well
...
By default, `fzf` install a very light wrapper on top of the `fzf` executable,
so I suggest you use `fzf.vim` which brings you nice commands out of the box
such as :
...
After installing the plug-in, all you have to do is to configure some mapping:
...
There's something really nice about using the same tool in my shell and in my
editor. It means I only need to learn how to use and configure it once.

Also, in my setup:

* The `:Files` command uses `<leader>t` in neovim, and that the equivalent
...
editing](http://moolenar.net/habits.html) article by Bram Moolenaar, this is no coincidence either :)
